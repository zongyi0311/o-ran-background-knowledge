...
| 子圖 | 條件說明                                                                                     |
|------|----------------------------------------------------------------------------------------------|
| (A)  | 經典多普勒模型，$\sigma_{f_D} = 70\,\mathrm{Hz}$，觀察時間 $T = 250\,\mathrm{ms}$                |
| (B)  | 同上但觀察時間縮短為 $T = 25\ \text{ms}$，可見時間域快速變化                                |
| (C)  | $\sigma_{f_D} = 20\ \text{Hz}$ 且包含一個靜止（zero-Doppler）通道分量，$T = 250\ \text{ms}$ |
| (D)  | 同上但觀察時間縮短為 $T = 25\ \text{ms}$，通道變化非常平穩                                  |
